...
function base_url()
{
	return 'http://localhost/cdm/index.php';
}
...
{
	$output = '';
	$query = "SELECT dealership_currency FROM setting LIMIT 1";
	...
	{
		if($currency["code"] == $result['dealership_currency'])
		{
...
function currency_array()
	...
	function Count_total_soldcars_number($connect)
{
	$total = 0;

	$query = "SELECT COUNT(sale_id) AS Total FROM sale";
...
function Count_total_cars_number($connect)
{
	$total = 0;

	$query = "
	SELECT COUNT(car_id) AS Total FROM car 
...
function Count_total_available_cars_number($connect)
{
	$total = 0;
...
function Count_total_revenue_Generated($connect) {
    $query = "SELECT SUM(sale_price) as total FROM sale";
    $statement = $connect->prepare($query);
    $statement->execute();
    $result = $statement->fetch(PDO::FETCH_ASSOC);
    return $result['total'];
}
function Count_total_salesman_number($connect)
{
	$total = 0;

	$query = "
	SELECT COUNT(salesperson_id) AS Total FROM salesperson 
